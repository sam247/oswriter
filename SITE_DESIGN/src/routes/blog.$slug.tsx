...
export const Route = createFileRoute("/blog/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    if (!post) {
      return { meta: [{ title: "Article not found — QueueWrite" }] };
    }
    return {
      meta: [
  ...
  notFoundComponent: () => (
    <div className="grid min-h-screen place-items-center bg-[#f5f5f7] px-4 text-center">
      <div>
        <h1 className="text-[28px] font-semibold tracking-tight">Article not found</h1>
        <p className="mt-2 text-[14px] text-[#4a4a4a]">It may have been moved or unpublished.</p>
        <Link to="/blog" className="mt-6 inline-flex h-10 items-center gap-2 rounded-full bg-[#0a0a0a] px-4 text-[13.5px] font-medium text-white">
  ...
  errorComponent: ({ reset }) => (
    <div className="grid min-h-screen place-items-center bg-[#f5f5f7] px-4 text-center">
      <div>
        ...
        <button onClick={reset} className="mt-6 inline-flex h-10 items-center gap-2 rounded-full bg-[#0a0a0a] px-4 text-[13.5px] font-medium text-white">
          Try again
        </button>
      </div>
    </div>
